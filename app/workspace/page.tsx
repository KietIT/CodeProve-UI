...
const levels = [
  { name: "Fresher", slug: "fresher", level: "Easy", filled: 1, count: 12, desc: "Foundational problem-solving with AI guidance. Build core habits: read, hypothesise, prompt." },
  { name: "Junior", slug: "junior", level: "Medium", filled: 2, count: 10, desc: "Intermediate logic and debugging. Verify AI output and explain your reasoning back." },
  { name: "Senior", slug: "senior", level: "Hard", filled: 3, count: 8, desc: "Advanced architecture and edge-case reasoning. High-stakes verification and testing." },
];
    ...
    <div className="flex min-h-screen flex-col bg-background font-body-md text-on-surface">
      ...
      <main className="mx-auto w-full max-w-container-max flex-1 px-5 py-10 md:px-12">
        <header className="mb-8">
          <span className="font-label-caps text-label-caps uppercase tracking-[0.2em] text-primary">Workspace</span>
          <h1 className="mt-2 font-headline-xl text-[40px] leading-none tracking-tight sm:text-headline-xl">Choose a challenge</h1>
          <p className="mt-3 max-w-lg text-on-surface-variant">Pick a training level to enter the assessment workspace. New exercise sets are being curated.</p>
        </header>
        ...
        <div className="ice-card mb-10 flex flex-wrap items-center gap-4 p-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-label-mono text-label-mono uppercase text-on-surface-variant/70">Tags:</span>
            {tags.map((t, i) => (
              <button
                key={t}
        ...
        <section className="mb-12">
          <div className="mb-5 flex items-center gap-4">
            <h2 className="font-headline-lg-mobile text-headline-lg-mobile">Training levels</h2>
            <div className="h-px flex-1 bg-outline-variant/60" />
            <span className="font-label-mono text-label-mono text-on-surface-variant/60">3 TRACKS</span>
          </div>
            ...
            {levels.map((lv) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-label-mono text-label-mono uppercase text-primary">{lv.level}</span>
                ...
                <p className="mt-2 flex-1 text-sm leading-relaxed text-on-surface-variant">{lv.desc}</p>
                <div className="mt-5 flex items-center justify-between border-t border-outline-variant/40 pt-4">
                  <span className="font-label-mono text-label-mono text-on-surface-variant/60">{lv.count} exercises</span>
                  <span className="flex items-center gap-1 font-label-mono text-label-mono text-primary transition-transform group-hover:translate-x-1">
                    Browse <Sym name="arrow_forward" className="text-[16px]" />
                  </span>
                </div>
              </Link>
            ))}
        ...
        <section>
          <div className="mb-5 flex items-center gap-4">
            <h2 className="font-headline-lg-mobile text-headline-lg-mobile">Business problems</h2>
            <span className="border border-primary/30 bg-primary/5 px-2 py-0.5 font-label-mono text-label-mono uppercase text-primary">Coming soon</span>
            <div className="h-px flex-1 bg-outline-variant/60" />
          </div>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {business.map((b) => (
              <div key={b} className="ice-card flex cursor-not-allowed flex-col p-5 opacity-70">
                ...
                <div className="mt-1 text-sm text-on-surface-variant/70">Real-world scenarios</div>
                <div className="mt-4 border-t border-outline-variant/40 pt-3 font-label-mono text-label-mono uppercase text-on-surface-variant/50">Locked</div>
              </div>
            ))}
          </div>
        </section>
